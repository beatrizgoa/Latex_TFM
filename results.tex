...
The cost and error obtained at training and validation processed when CASIA video database is used, are represented in figure \ref{fig:ejecucion2_casia_vid_train}. The minimun cost is almost 0 and is obtained before 2000th iteration, as is represented in figure \ref{fig:ejecucion2_casia_vid_train_cost}}. The validation converges at the same time as the training, where oscillate between 7\% an 6\%.\\
\begin{figure}[htb]
\centering
...
From the table it is not possible to know if LDA and PCA are significant because with some classifiers APCR and BPCR have been improve or worsened.\\
...
APCR and BPCR results are in table \ref{APCR_BPCR_FRAV}. In general, results are very good because values are lower than 0.5. With SVM linear kernel, all the samples are classified as negatives except when it has been used when LDA, that results are as good as the obtained with RBF kernel.\\
...
The misclassified samples are repeated among the classifiers. The two samples shown in figure: %('False Positive: ', 'databases/from/attack_04/tablet_162.JPG') ('False Positive: ', 'databases/from/attack_04/tablet_109.JPG')
represents the two most misclassified samples, almost every classifier have classified it incorrectly.\\

\subsection{FRAV RGB+NIR (feature level) database}
Results obtained at classifying RGB+NIR (feature level) database are shown following.\\

Table \ref{APCR_BPCR_FRAV_feature} shows the APCR and BPCR values that have been obtained for each classifier.
\begin{table}[htb]
\centering
\resizebox{\textwidth}{!}{%
\begin{tabular}{|c|c|c||c|c|c||c|c|c|}
...
\begin{tabular}[c]{@{}c@{}}Decision Tree\\ Depth =  2\end{tabular}           & \texttbf{0}    & \textvf{0}    & \begin{tabular}[c]{@{}c@{}}Decision Tree\\ Depth = 2\end{tabular}              & 0.01 & 0    & \begin{tabular}[c]{@{}c@{}}Decision Tree\\ Depth = 2\end{tabular}            & 0.05 & 0    \\ \hline
