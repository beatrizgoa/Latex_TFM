...
From the general experiment, it can be seen that the cost converges in low values, the validation error converges (from a 10\% error to very low values near to 0\% error) for all databases except for MFSD-MSU database as could be seen in figure \ref{fig:mfsd-ejec1} which means that the generalization is not correct. \\
...
\subfigure[RGB Error]{\includegraphics[width=0.48\textwidth]{images/images_ejecucion1/RGB_FRAV_CLAS_experiments_error.png} \label{fig:RGB_FRAV_CLAS-ejec1_error} }
...
\subfigure[NIR Error]{\includegraphics[width=0.48\textwidth]{images/images_ejecucion1/NIR_FRAV_CLAS_experiments_error.png} \label{fig:NIR_FRAV_CLAS-ejec1_error} }
...
\section{Description of the final experiment}\label{sec:Final_archi} %Used in ejecucion 2
